<template lang='pug'>
        div( style="background:#f7f7f7;height:100%;position: relative;" )
            p.Title {{Title}}
                span >
                span.list {{list}}
            div(style="margin:0 auto;width:900px;height:100%;" v-show="firstStep")
                div
                    img(style="width:900px;" src="../assets/Home/flow1.png")    
                ...
                .InputList
                    div
                        label 经纬度
                        Input(v-model="longitudelatitude" placeholder="请点击打开地图"  @on-focus="Positioning"  style="width: 70%")
                    div
                      label 商品类型
                ...
                .InputList
                    div  
                        label 商品是否显示
                        Select(style="width: 70%"  v-model="isShowIndex")
                          Option(key="0" value="1") 是
                          Option(key="1" value="0") 否                                                    
                    div
                //-         label 第三方平台
                ...
                div(style="padding-top:30px;padding-bottom: 15px;background:#fff;text-align:center;")
            ...
            div(v-show="SecondStep" style="margin:0 auto;width:900px")
                ...
                div(style="height:150px;margin-top:30px;background:#fff" )
                    div(style="float:left;")
                      h3(style="font-weight: bold;") &nbsp; 海报图片（比例：6 : 7.5）
                      Upload(
                            ...
                            img(:src="picturePoster" style="width:100px;height:120px;")
                    //- div(style="float:left;margin-left:8px;")
                    //-   h3(style="font-weight: bold;") 封面图片：
                    //-   div(style="width:100px;height:100px;border: 1px solid #DCDCDC;")
                    //-     Upload( :before-upload="productCoverUpload" :show-upload-list="false" action="//jsonplaceholder.typicode.com/posts/")
                    //-       img(:src="productCoverImg" style="width:100px;height:100px;")
                div(style="background:#fff;padding-bottom:8px;padding-left:2px;clear:both;" )
                ...
                div(style="background:#fff;margin-top:10px; z-index: 0;"  ref="editor")      
                ...
                div(style="margin-top:30px;background:#fff" )
                        ...
                        div( class="Package") 
                          .packageTop {{packageTop}}
                            Button(@click="close" style="float:right;")
                              Icon(type="close")
                          div(class="SetMeal" ref="packageKu")
                            .InputList(style="margin-top:10px;")
                              div
                                  label 套餐名称
                                  Input( :style="[isB?style0: style1]" placeholder="不超过20个字" v-model="Package.packageName")
                              div
                                  label 退款条件
                            ...
                            .InputList(style="margin-top:10px;")
                              div
                                  label 销售价
                                  InputNumber( :disabled="packageDisabled" style="width: 70%" v-model="Package.salePrice")
                              div
                                  label 结算价
                                  InputNumber(  style="width: 70%" v-model="Package.settlementPrice") 
                              div
                                  label 平台佣金
                                  InputNumber(:style="[isC?styleP: style1]" :disabled="packageDisabled" style="width: 70%" v-model="Package.platformCommission")
                              div
                                  label 默认代理佣金
                            ...
                            .InputList(style="margin-top:10px;" )
                              div
                                  label 设置销售时间
                                  Select(style="width: 70%"  v-model="Package.isFlashSale")
                                    Option(key="0" value="0") 否
                                    Option(key="1" value="1") 是                                             
                            .InputList(style="margin-top:5px;" v-if="Package.isFlashSale=='1'")
                              div
                                  label 销售开始日期
                                  DatePicker( type="datetime" format="yyyy-MM-dd HH:mm" style="width: 70%" v-model="Package.saleStartDate")
                              div
                                  label 销售结束日期
                                  DatePicker(  type="datetime" format="yyyy-MM-dd HH:mm" style="width: 70%" v-model="Package.saleEndDate")   
                            .InputList(style="margin-top:10px;" )
                              div
                                  label 门票有效时间
                                  Select(style="width: 70%"  v-model="Package.saleDateType")
                                    Option(key="0" value="NONE") 永久有效
                                    Option(key="1" value="RANGE") 日期区间
                                    Option(key="2" value="WEEK") 周期
                                    Option(key="3" value="TODAY") 限购票指定日
                            .InputList( v-if="Package.saleDateType=='RANGE'")
                              div
                                  label --开始日期
                            ...
                            .InputList(v-if="Package.saleDateType=='WEEK'")
                                div
                                    label --选择时间
                            ...
                            .InputList( v-if="Package.saleDateType=='WEEK'")
                              div
                                  label --开始日期
                                  DatePicker(  type="date" style="width: 70%" :disabled="dataDisabled" v-model="Package.startDate")
                              div
                                  label --结束日期
                                  DatePicker(  type="date" style="width: 70%" :disabled="dataDisabled" v-model="Package.endDate")
                            .InputList()
                              div
                                  label 截止售票时间
                            ...
                            div(style="margin:5px 0 0 5px;")
                                    label(style="display: inline-block;width: 72px;") 第三方平台
                            ...
                            div
                                h3(style="font-weight: bold;margin-top:10px;")  &nbsp;点击上传海报图片：用于海报链接（宽高比 6：7.5） 
                                Upload(
                            ...
                            div(style="height:35px;")     
                              h3(style="font-weight: bold;float:left;padding-top:8px;") &nbsp; 费用须知  
                              Button(type="success" @click="preview0" style="float:right;margin-right:10px;")  预览内容 
                            div(style="margin-top:10px;position: relative;z-index: 1;")
                              div(ref="editors" )
                            div(style="text-align:center;padding:5px 0;")
                              Button(type="success" :loading="packageLoading" @click="Submitpackage") 提交
                            Modal(v-model="productInfoShow0" title="详情" width="740px")
                                p(ref="edit0")
                    div(style="margin-top:30px;" @click.stop="clicktable" )
                        ...
                        div(style="margin-top:30px;text-align: center;padding-bottom:30px;")
                            ...
                            Button(type="success" @click="jumpProduct0"  :loading="jumpLoading" style="margin:0 10px;") 直接上架 
                                                                                   
</template>

<script>
import Updataimg from "./Updataimg.vue";
import { add0, getDate, withinErrorMargin, hhssmm, accAdd,shm,dataTime } from "../js/public.js";
import "animate.css";
...
export default {
  ...
  data() {
    return {
      dataDisabled: true,
      productCover: "",
      productCoverImg: require("../assets/Home/a11.png"),
      spinShow: false,
      ...
      province: [],
      longitudelatitude: "",
      picturePoster: require("../assets/Home/a11.png"),
      coverImg: "",
      productInformation: {
        productName: "",
        address: "",
        ...
        },
        {
          ...
          render: (h, par) => {
            if(par.row.isFlashSale == "0"){
              ...
              return dataTime(par.row.saleStartDate	) + " 至 " + dataTime(par.row.saleEndDate)
            }
          }
        },
        {
          ...
          render: (h, par) => {
            if(par.row.saleDateType == "NONE"){
              ...
              return getDate(par.row.startDate) + " 至 " + getDate(par.row.endDate)
              ...
              return weekStr = weekStr.substring(0, weekStr.length - 1)
  ...
  methods: {
      ...
      this.axios
        ...
        .then(res => {
          if (res.errorCode == 200) {
            ...
            this.productCover = res.data;
            reader.readAsDataURL(file);
            reader.onload = function(e) {
              _this.productCoverImg = this.result;
            };
          }else{
            this.error("图片上传错误");
            this.productCoverImg = require("../assets/Home/a11.png");
          }
        })
        .catch(err => {});
    },
    ...
    jumpProduct0() {
      ...
      if(list.length == 0){
        ...
        this.axios
          .post(`${this.AjaxUrl}/ty_business/product/insertPut`, this.qs.stringify(data))
          .then(res => {
            if (res.errorCode == 200) {
              console.log(res);
              this.success("上架成功");
    ...
    handleUploadPoster(file) {
      ...
      reader.readAsDataURL(file);
      reader.onload = function(e) {
        _this.picturePoster = this.result;
      };
      return false;
    },
    //上传套餐海报图片
    ...
    Submitpackage(Listtype = 1, productId = "") {
      ...
      // if (this.Package.packageName.length > 20) {
      //     this.error("套餐名称不超过20字");
      //     return false;
      // }
      if(this.imglist0 == "") {
        this.error("请上传海报图片!");
      ...
      if(isFlashSale == "1") {
        data.saleStartDate = dataTime(saleStartDate);
        data.saleEndDate = dataTime(saleEndDate);
      }
    ...
    //清空套餐
    Newpackage() {
      this.$refs.packageKu.scrollTop = 0;
      this.isA = false;
      this.isB = false;
      this.isC = false;
      this.packageLoading = false;
    ...
    PackageEditor(index) {
      ...
      this.imglist0 = coverImg;
      this.productPackageId = productPackageId;
      this.Package.isFlashSale = isFlashSale.toString();
      if(isFlashSale == "1") {
        this.Package.saleStartDate = dataTime(saleStartDate);
        this.Package.saleEndDate = dataTime(saleEndDate);
      }
    ...
    packageremove(index) {
      this.$Modal.confirm({
        ...
        onOk: () => {
          ...
          data.productId = this.ProductList[index].productId;
          console.log(data);
          this.axios
            .post(url, this.qs.stringify(data))
            .then(res => {
    ...
    CreateAproduct(type = 1) {
      ...
      data.isShowIndex = this.isShowIndex;
        // if (this.ZYBtype) {
        //   let { apiCode, isRealName } = this;
        //   if (!apiCode || !isRealName) {
        //     this.error("所有内容都必须填写!");
  ...
  watch: {
    // "Package.saleDateType": {
    //   handler: function(val, oldVal) {
    //     if (this.Package.saleDateType == "WEEK") {
    //       this.Package.startDate = "";
    //       this.Package.endDate = "";
    //       return false;
    //     }
    ...
    "Package.salePrice": {
      handler: function(val, oldVal) {
        ...
        if(this.productPackageId == ""){
          ...
          this.Package.platformCommission = Math.round(data);
          return this.Package.platformCommission;
        }else {
          return this.Package.platformCommission;
        }
      },
    ...
    "Package.platformCommission": {
      handler: function(val, oldVal) {
        ...
        if(this.Package.platformCommission < data) {
          this.isC = true;
