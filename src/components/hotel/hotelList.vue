  ...
  <div  class="Box_list">     
         <div  v-for="(list,index) of DataList"  :key="index"  style="width:49%; display:inline-block;margin-bottom:8px;">
            <div class="Commoditylist">
             ...
             <div class="CommodityBody">
                 ...
                 <div class="CommodityBottom1">
                      <p>酒店地址：</p>
                      <p :title="list.address">{{list.address}}</p>
                 </div>
             </div>
             </div>
        ...
        <!-- 酒店详情 -->
          <div v-show="packageShow" style="position: fixed;overflow: auto;top: 0;right: 0;bottom: 0;width:100%;height:100%;background: rgba(0,0,0,.5);z-index:90;">
            <transition  enter-active-class="animated zoomInLeft" leave-active-class="animated zoomOutRight">
              <hotelDetails  v-show="packageShow"  :hotelId="productIdList" :series="series"  @closechang="closechang"></hotelDetails>
            </transition>
          </div>
          <!-- 上下架 删除   -->
          <Modal
...
export default {
  ...
  methods: {
    ...
    ok() {
      ...
      } else if(this.operatingState == 0){
        url = `${this.AjaxUrl}/ty_business/hotel/open`;
      }else if(this.operatingState == 1){
        url = `${this.AjaxUrl}/ty_business/hotel/close`;
      }
      this.axios
