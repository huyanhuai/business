  ...
  <div class="packageAdd">
    ...
    <div style="width:100%;height: 650px;overflow: auto;" ref="kuang">        
      ...
      <div class="InputList">
        <div>
            <label for="">名称</label>
            <Input v-model="productInformation.productName" placeholder="不超过20个字"  style="width: 70%"></Input>
        </div>
        <div>
            <label for="">详细地址</label>
            <Input v-model="productInformation.address"  style="width: 70%"></Input>
        </div>
      </div>
      ...
      <div class="InputList">
        ...
        <div>
            ...
            <Select v-model="productPackageTypeId"  multiple style="width: 70%">
              <Option  v-for="item in PackageType"  :key="item.productPackageTypeId" :value="item.productPackageTypeId">{{item.typeName }}</Option>
            </Select>
        </div>
      </div>
      <div class="InputList">
        <div>
            <label for="">地区</label>
            <Select v-model="productInformation.provinceId" style="width: 23.333%">
              <Option v-for="item in province"  :key="item.name" :value="item.name">{{item.label }}</Option>
            </Select>
            <Select v-model="productInformation.cityId" style="width: 23.333%">
              <Option v-for="item in cityList"  :key="item.name" :value="item.name">{{item.label }}</Option>
            </Select>
            <Select v-model="productInformation.areaId" style="width: 23.333%">
              <Option v-for="item in categoryList"  :key="item.name" :value="item.name">{{item.label }}</Option>
            </Select>
        </div>
        <div>
            <label for="">产品副标题</label>
            <Input v-model="productInformation.productTitle" placeholder="不超过30个字"  style="width: 70%"></Input>
        </div>
      </div>

      <div class="InputList">
        <div>
            <label for="">商品是否显示</label>
            <Select v-model="isShowIndex" style="width: 70%">
      ...
      <div style="margin-top:20px;background:#fff"> 
        <h3 style="font-weight: bold;">产品轮播图</h3>
          ...
          <Upload  style="display:inline-block" multiple :before-upload="handleUpload" :show-upload-list="false" action="//jsonplaceholder.typicode.com/posts/">
            <img src="./../../assets/Home/a11.png" style="width:100%">
          </Upload>
      </div>
    ...
    series: function(val) {
      ...
      this.axios
        ...
        .then(res => {
          ...
          this.isShowIndex = isShowIndex.toString();
          ...
          for (let i of productPictureList) {
            // if(i.productPictureId == 340){
            //   this.imglist1.unshift({ url: i.pictureUrl });
            // }else{
...
.demo-upload-list-cover i:nth-child(1) {
  font-size: 20px;
}
#keyword{
  width: 30%;
