      ...
      <div  v-show="ShowCode2" class="SetMeal">
        ...
        <div v-show="ShowCode4">
          <div  class="SetForm">
            <div>
               <label>套餐名称</label>
                <Input v-model="Package.packageName" placeholder="不超过20个字"  class="width70"  :style="[isB?style0: style1]"></Input>
            </div>
            <div>
          ...
          <div class="SetForm">
            <div>
               <label>平台佣金(元)</label>
               <Input :style="[isC?styleP: style1]" :disabled="packageDisabled" v-model="Package.platformCommissionRatio" style="width:70%"></Input>
               <!-- <span class="platNum">{{platform}}</span> -->
            </div>
            <div>
               <label style="width:95px">默认代理佣金(元)</label>
                <Input v-model="Package.defaultAgentCommissionRatio"  style="width:63%"></Input>
                <!-- <span class="commission">{{defaultAgent}}</span> -->
            </div>
            <div>
               <label style="width:95px">默认业务佣金(元)</label>
               <Input v-model="Package.defaultSaleCommissionRatio"  style="width:63%"></Input>
          ...
          <div  class="SetForm">
              <div>
               <label>设置销售时间</label>
               <Select v-model="Package.isFlashSale"  style="width:70%;">
          ...
          <div class="InputList InputList1"  v-if="Package.isFlashSale=='1'">
              <div class="StopTime">
                <label for="">销售开始日期</label>
                <DatePicker type="datetime" format="yyyy-MM-dd HH:mm" style="width: 70%" v-model="Package.saleStartDate"></DatePicker>
              </div>
              <div class="StopTime">
                <label for="">销售结束日期</label>
                <DatePicker type="datetime" format="yyyy-MM-dd HH:mm" style="width: 70%" v-model="Package.saleEndDate"></DatePicker>
              </div>              
          </div>  
          ...
          <div class="InputList" v-if="Package.saleDateType == 'WEEK'">
              <div>
                <label for="">--开始日期</label>
                <DatePicker type="date" v-model="Package.startDate" :disabled="dataDisabled" placeholder="请选择日期" style="width: 70%"></DatePicker>
              </div>
              <div>
                <label for="">--结束日期</label>
                <DatePicker type="date" v-model="Package.endDate" :disabled="dataDisabled" placeholder="请选择日期" style="width: 70%"></DatePicker>
              </div>            
          </div> 
          ...
          <div>
          <h3 style="font-weight: bold;padding:10px 0;">点击上传海报图片：用于海报链接（宽高比 6：7.5）</h3> 
        <Upload
      ...
      <div v-show="ShowCode3" style="padding: 0 20px">
            <div  class="SetForm" style="margin:0px 0px 8px;padding-top:8px;">
                ...
                <div>
                  ...
                  <Select class="width70"   v-model="commissionForm.agentUserInfoId">
                      <Option v-for="(item,index) in commissionAgents" :key="index" :value="item.id">{{item.name}}</Option>
                  </Select>
                </div>                
            </div>
...
import { add0, getDate, withinErrorMargin, hhssmm, accAdd, accSub,shm,dataTime } from "../../js/public.js";
...
export default {
  ...
  methods: {
    ...
    commissionAgentsList(index = 0) {
      this.axios
        .get(
          `${this.AjaxUrl}/ty_business/product/package/getAgentPackageListByBusiness`,
          {
            params: {
              productId: this.productId,
          ...
          if (res.errorCode == 200) {
            ...
            for(let i of res.data.list){
              if(i.companyName != null && i.companyName != ""){
                this.commissionAgents.push({
                  id: i.agentUserInfoId + "," + i.userProductId,
                  name: i.companyName
                });
              }else {
                this.commissionAgents.push({
                  id: i.agentUserInfoId  + "," + i.userProductId,
                  name: i.legalRepresentativeName
                });
              }
            }
            console.log(this.commissionAgents);
            //console.log(res);
          }
        });
    },
    ...
    Commission() {
      this.Btnclick=true;
        if (this.commissionForm.agentUserInfoId == "") {
          this.error("请选择代理商");
          return false;
        }
      let {
      ...
      data.agentUserInfoId = id[0];
      data.productPackageId = productPackageId;
      data.userProductId = id[1];
    ...
    packageremove(index) {
      const title = "警告";
      let content = "<p>确认删除该套餐吗？</p>";
      if(this.ProductList.length == 1 && this.status == 1) {
        content = "<p>该套餐删除后上架产品将自动下架，确认删除该套餐吗？</p>";
      }
      this.$Modal.confirm({
        title: title,
        content: content,
